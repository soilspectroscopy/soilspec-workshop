...
Except for `delta.wav`, we can fine tune all of these parameters to find the best preprocessing combination, although several studies have already explore this in the literature and we can adopt what has been recommended. In this example, we are going to use the first-derivative of a second order polynomial regression with half-window size of 11 nm.
...
```
Moving-window first-derivatives are great because they preserve the orientation of the absorption features and enhance only the important regions of the spectra. For more complex spectra, however, this may hamper the interpretability.

::: {.callout-tip}
## Tip
**dplyr** allows us to use column selectors like `first()`, `everything()`, `all_of` etc. As we now have a shorter spectral range, rather than using `all_of(new.spectra.column.names)` we can replace it with `any_of(new.spectra.column.names)` to select any of the available columns within the predefined range of columns.
:::
